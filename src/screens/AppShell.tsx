...
export default function AppShell() {
  ...
  return <NavigationContainer><Stack.Navigator initialRouteName="Dashboard" screenOptions={screenOptions}>
    ...
    <Stack.Screen name="AccountForm">{({navigation}) => <AccountFormScreen record={selected} data={data} onBack={() => navigation.goBack()} onCreateRelated={next => {persist(next);}} onSave={async value => {await save(value); navigation.navigate('AccountDetail');}}/>}</Stack.Screen>
    ...
    <Stack.Screen name="FilingDetail">{({navigation}) => selectedFiling ? <FilingDetailScreen data={data} entity={selectedFiling.entity} kind={selectedFiling.kind} onBack={() => navigation.goBack()}/> : null}</Stack.Screen>
